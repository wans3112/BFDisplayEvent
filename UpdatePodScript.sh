...
pod lib lint --sources=$privateSpecs,$publicSpecs --verbose --use-libraries --allow-warnings

# --allow-warnings : 允许 警告，有一些警告是代码自身带的。
# --use-libraries  : 私有库、静态库引用的时候加上
# —verbose ： lint显示详情

pod repo push WBSpecs $podspecName.podspec --sources=$privateSpecs,$publicSpecs --use-libraries --allow-warnings
